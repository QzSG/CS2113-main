...
```
. Inside each of the backup tasks, the specific online service that implements the `OnlineStorage` interface will be called.
...
. When each backup task is completed, a `OnlineBackupSuccessResultEvent` is raised by the `setOnSucceeded` task handler.
